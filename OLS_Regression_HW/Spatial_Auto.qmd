...
**Studentized Breusch-Pagan Test Interpretation:** Like the previous test, it observes for any heteroscedasticity, the only difference being that the original assumes normal distribution off the OLS residual values. However, the Studentized version is less stringent as it assumes non-normal distribution and adjusts for variance, this makes it better to use when certain data is known to be highly skewed, like this paper's analysis of median income with housing prices and vacancies in Philadelphia. Again, with this output, the CP is 20.892 with a very small p-value, meaning reject the null hypothesis, and the data is heteroscedastic.
